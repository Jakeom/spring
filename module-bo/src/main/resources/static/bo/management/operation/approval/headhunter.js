var table = $("#datatable").DataTable({
	ajax: {
		url: "/bo/management/operation/approval/headhunter/list",
			...
			render: function(data, type, row) {
				if(row.status == 'RECEIPT') {
					return '<button name="'+ row.id + '" data-toggle="modal" data-role="nameCheck" data-target="#refuseModal" class="btn btn-outline-secondary">거절</button> '
						+ '<button data-member-id="' + row.memberId + '" data-role="approval" name="' + row.id + '"class="btn btn-outline-danger">승인</button>'
				}
				else{
					return '-';
				}
...
function getMemberId(id){
	...
	$.ajax({
		type: "get",
		url: "/bo/management/operation/approval/headhunter/hhRequestDetail",
		data: {
			memberId : id,
		},
		dataSrc: function(data) {
		...
		success : function (data) {
			...
			let append = '<tr><th>접수내역</th><td>' + 'HH회원가입' + '</td></tr><tr><th>접수일시</th><td>' + data.data.createdAt + '</td></tr><tr><th>이름</th><td>' + data.data.memberName +
...
$("button[data-role=today]").click(function() {
	...
	$('#hhStartDt').datepicker('setDate', start);
	$('#hhEndDt').datepicker('setDate', end);
})
...
$("button[data-role=all]").click(function() {
	...
	$('#hhStartDt').datepicker('setDate', null);
	$('#hhEndDt').datepicker('setDate', null);
})

$('#hhStartDt').change(function() {
	start = $(this).val();
})

$('#hhEndDt').change(function() {
	end = $(this).val();
})
...
$(document).on("click", "button[data-role=approval]", function () {
	...
	if(!confirm("승인하시겠습니까?")) {
		...
		$.ajax({
			url: "/bo/management/operation/approval/headhunter/accept",
			type: "post",
...
$(document).on("click", "button[data-role=modalSave]", function () {
	...
	$.ajax({
		url: "/bo/management/operation/approval/headhunter/reject",
		type: "post",
		contentType: "application/json",
		data: JSON.stringify(params),
		success: function () {
...
function loadTable() {
	...
	let admin = $("input[data-role=admin]").val();
	...
	$("#datatable").DataTable({
		...
		ajax: {
			type: "post",
			url: "/bo/management/operation/approval/headhunter/search",
			data: params,
			dataSrc: function(data) {
				return data.data;
			}
