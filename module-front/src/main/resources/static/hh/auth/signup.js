...
// 약관 동의시 이벤트 전파 방지
$("#nes2, #nes3, .form-check-label").click(function(e){
    e.stopPropagation();
});
...
$("button[data-role=submit]").click(function(){
    ...
    obj.sfWorkerListFileId = $("input[name=sfWorkerListFileId]").val();
    obj.sfPhone = $("input[name=sfPhone]").val();
    obj.sfRegNumber = $("input[name=sfRegNumber]").val();
    obj.sfCeoName = $("input[name=sfCeoName]").val();
    obj.sfName = $("input[name=sfName]").val();
    ...
    let fileCheckObj = Utils.fileCheck($("input[name=files]"), 10, false);
    ...
    if(!$("#nes2").is(":checked") || !$("#nes3").is(":checked")){
        alert("필수 이용약관을 동의해주셔야 합니다.");
        return false;
    }

    let form = $("form[name=insertForm]")[0];
    let formData = new FormData(form);

    formData.append("agreeMarketing", ($("input[name=agreeMarketing]:checked").val() == "Y" ? 1 : 0));
    formData.append("agreeAd", $("input[name=agreeAd]:checked").val() == "Y" ? 1 : 0); // 광고 정보 수신 동의
            ...
            if(d.data){
                ...
                $.ajax({
                    ...
                    success: function (d) {
                        if(d.serviceCode){
                            alert("회원가입이 완료되었습니다.");
                            location.replace("/hh/auth/login");
                        } else {
