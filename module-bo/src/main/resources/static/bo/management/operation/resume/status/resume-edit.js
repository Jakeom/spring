...
let addEducation = $('div.cloneArea > div.university-area').clone();
let addPortfolio = $('div.cloneArea > div.portfolio-list').clone();
$(".university-area:last").remove()
let deleteAcademicList = [];
let deleteCareerList = [];
let deletePortfolioList = [];
...
$("[data-role=deleteEducation]").click(function(){
    let eduId = nvl($(".university-area:last").find('input[name=id]').val())
    if(eduId !=""){
        deleteAcademicList.push(eduId)
    }
    $(".university-area:last").remove();
});
...
$("[data-role=deletePortfolio]").click(function(){
    let portfolioId = nvl($(".portfolio-area > div:last").find('input[name=id]').val())
    if(portfolioId !=""){
        deletePortfolioList.push(portfolioId)
    }
...
$("button[data-role=careerSave]").click(function (){
    ...
    //각각 경력
    let resYm = moment(resignationYms.val(), "YYYYMM");
    ...
    if(mode == "INSERT"){
        ...
        careerSaveBlock.find('.careerPeriod').text(Math.floor(careerMonth / 12) + "년 " + (careerMonth % 12) + "개월");
        ...
        career.find('.careerPeriod').text(Math.floor(careerMonth / 12) + "년 " + (careerMonth % 12) + "개월");
        career.find('.company').text(companyNames.val());
        career.find('.entranceYms').text(entranceYms.val());
        career.find('.position-input').text(positionInputs.val() + " / " + dutyInputs.val());
        ...
        if(holdOffices.val() == '1') {
            career.find('.holdOffices').text(holdOffices.val());
        } else {
    ...
    // 경력 계산
    let month = 0;
    $("div.career-area > div.career").each(function(){
...
$(document).on("click", "[data-role=careerDelete]", function (){
    let careerId = nvl($(this).closest(".career").find('input[name=id]').val())
    if(careerId !=""){
        deleteCareerList.push(careerId)
    }
...
$(document).on("change", ".portfolio-files", function(e){
    ...
    $(this).closest(".portfolio-file").find("input[name=fileChangeFlag]").val("Y");
});
...
$(document).on("change", "#enResumeFile", function(e){
    ...
    $("input[name=enFileChangeFlag]").val("Y");
});
...
$('button[data-role=save]').click(function (){
    ...
    let graduationStatusCd = $(".high-school-area input[name=graduationStatusCd]");
    ...
    let flag = true;
    $("div.desc > .add-education").each(function(){
    ...
    let jsonData = {
        'id': $('#resumeId').val() ,
        'photoChangeFlag': $("input[name=photoChangeFlag]").val(),
        'enFileChangeFlag': $("input[name=enFileChangeFlag]").val(),
        ...
        'deleteAcademicList': deleteAcademicList,
        'deleteCareerList': deleteCareerList,
    ...
    academicBackgroundList.push({
        'id': $('.high-school-area input[name=id]').val(),
        'degreeCd':  'HIGH',
        ...
        'graduationYm':  $('.high-school-area input[name=graduationYm]').val().replace('.',''),
    ...
    // 대학교 이상
    $('div.desc > div.university-area').each(function(){
        academicBackgroundList.push({
            'id': $(this).find('input[name=id]').val(),
            'sqlType' : $(this).find('input[name=sqlType]').val(),
            ...
            'graduationYm':  $(this).find('input[name=graduationYm]').val().replace('.',''),
            'entranceYm':  $(this).find('input[name=entranceYm]').val().replace('.',''),
    ...
    // 포트폴리오
    let portfolioList = [];
    $("div.portfolio-area > div.portfolio-list").each(function(){
        portfolioList.push({
            'id': $(this).find('input[name=id]').val(),
            'sqlType': $(this).find('input[name=sqlType]').val(),
            'fileChangeFlag': $(this).find('input[name=fileChangeFlag]').val(),
    ...
    // 전송
    $.ajax({
        url: '/bo/management/operation/resume/status/resume-edit/save',
        type: 'POST',
        data: formData,
        contentType: false,
        processData: false,
        ...
        success: function (data) {
            if(data.status == 200){
                ...
                location.href = "/bo/management/operation/resume/status/" + $("#resumeId").val()
            }
        }
    });
})
